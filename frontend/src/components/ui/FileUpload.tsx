  ...
  return (
    <div className="space-y-1">
      ...
      {value ? (
        ...
        <label className="flex cursor-pointer flex-col items-center gap-2 rounded-lg border-2 border-dashed border-(--border) p-6 transition-colors hover:border-(--accent-primary) hover:bg-(--bg-tertiary)">
          <Upload className="h-8 w-8 text-(--text-tertiary)" />
          <span className="text-sm text-(--text-secondary)">Click to upload</span>
          <span className="text-xs text-(--text-tertiary)">JPG, PNG or PDF (max 5MB)</span>
