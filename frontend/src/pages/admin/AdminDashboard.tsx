				...
				<>
					...
					<div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
						...
						<StatTile
							...
							sub={`${stats.merchants.active} active`}
						/>
						<StatTile
							label="Investments"
							value={stats.investments.total}
							sub={`${stats.investments.active} active`}
						/>
						<StatTile
							label="Pending KYC"
							value={stats.kyc.pending}
							sub={`${stats.kyc.resubmission_required} resubmissions`}
						/>

						<StatTile
							label="Total Invested"
							value={`\u20A6${stats.investments.total_invested.toLocaleString()}`}
						/>
						<StatTile
							label="Portfolio Value"
							value={`\u20A6${stats.investments.total_current_value.toLocaleString()}`}
						/>
						<StatTile
							label="KYC Approved"
							value={stats.kyc.approved}
							sub={`of ${stats.kyc.total}`}
						/>
					...
					<div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
							...
							<div className="space-y-2">
								{(
									[
										{
											label: "Active",
											count: stats.merchants.active,
											color: "bg-green-500",
										},
										{
											label: "Pending KYC",
											count: stats.merchants.pending_kyc,
											color: "bg-yellow-500",
										},
										{
											label: "KYC Submitted",
											count: stats.merchants.kyc_submitted,
											color: "bg-blue-500",
										},
										{
											label: "KYC Rejected",
											count: stats.merchants.kyc_rejected,
											color: "bg-red-500",
										},
										{
											label: "Suspended",
											count: stats.merchants.suspended,
											color: "bg-orange-500",
										},
										{
											label: "Inactive",
											count: stats.merchants.inactive,
											color: "bg-gray-500",
										},
									] as const
									...
									<div key={label} className="flex items-center gap-2 text-xs">
										...
										<div className="h-2 flex-1 overflow-hidden rounded-full bg-[var(--bg-tertiary)]">
											<div
												...
												style={{
													width: stats.merchants.total
														? `${(count / stats.merchants.total) * 100}%`
														: "0%",
												}}
											/>
										</div>
										<span className="w-6 text-right font-medium text-[var(--text)]">
											{count}
										</span>
									</div>
								))}
						...
						<Card padding={false} className="p-4">
							...
							<h2 className="mb-2 mt-4 text-sm font-semibold text-[var(--text)]">
								KYC Pipeline
							</h2>
							{stats.kyc.total > 0 && (
								<div className="mb-2 flex h-3 overflow-hidden rounded-full">
									<div
										...
										style={{
											width: `${(stats.kyc.approved / stats.kyc.total) * 100}%`,
										}}
									/>
									<div
										className="bg-yellow-500"
										style={{
											width: `${(stats.kyc.pending / stats.kyc.total) * 100}%`,
										}}
									/>
									<div
										className="bg-red-500"
										style={{
											width: `${(stats.kyc.rejected / stats.kyc.total) * 100}%`,
										}}
									/>
									<div
										className="bg-orange-500"
										style={{
											width: `${(stats.kyc.resubmission_required / stats.kyc.total) * 100}%`,
										}}
									/>
								</div>
							)}
							<div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px]">
								<span className="flex items-center gap-1">
									...
									Approved ({stats.kyc.approved})
								</span>
								<span className="flex items-center gap-1">
									<span className="inline-block h-2 w-2 rounded-full bg-yellow-500" />{" "}
									Pending ({stats.kyc.pending})
								</span>
								<span className="flex items-center gap-1">
									<span className="inline-block h-2 w-2 rounded-full bg-red-500" />{" "}
									Rejected ({stats.kyc.rejected})
								</span>
								<span className="flex items-center gap-1">
									<span className="inline-block h-2 w-2 rounded-full bg-orange-500" />{" "}
									Resubmit ({stats.kyc.resubmission_required})
								</span>
							</div>
						</Card>
							...
							<div className="space-y-2 text-xs">
								{(
									[
										...
										{
											label: "Active",
											value: String(stats.investments.active),
										},
										{
											label: "Withdrawn",
											value: String(stats.investments.withdrawn),
										},
									...
									<div key={label} className="flex justify-between">
										...
										<span
											className={`font-medium ${color || "text-[var(--text)]"}`}
										>
											{value}
										</span>
									</div>
