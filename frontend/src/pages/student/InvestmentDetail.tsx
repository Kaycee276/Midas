...
const InvestmentDetail = () => {
  ...
  if (!investment) return <div className="py-20 text-center text-[var(--text-secondary)]">Investment not found</div>;

  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      <Link to="/student/portfolio" className="mb-6 inline-flex items-center gap-1 text-sm text-[var(--text-secondary)] hover:text-[var(--text)]">
      ...
      <Card>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-[var(--text)]">{investment.merchant?.business_name || 'Investment'}</h1>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-[var(--text-secondary)]">Invested On</p>
              <p className="text-lg font-semibold text-[var(--text)]">{new Date(investment.invested_at).toLocaleDateString()}</p>
            </div>
          </div>
          <div>
        ...
        {investment.notes && (
          <div className="mt-4">
            ...
            <p className="text-[var(--text)]">{investment.notes}</p>
          </div>
        )}
      </Card>
