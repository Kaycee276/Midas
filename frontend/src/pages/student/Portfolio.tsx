    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mt-6 flex gap-2 border-b border-[var(--border)]">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium transition-colors ${tab === 'investments' ? 'border-b-2 border-[var(--accent-primary)] text-[var(--text)]' : 'text-[var(--text-secondary)]'}`}
        >
          Investments ({investments.length})
        </button>
        <button
          onClick={() => setTab('history')}
          className={`px-4 py-2 text-sm font-medium transition-colors ${tab === 'history' ? 'border-b-2 border-[var(--accent-primary)] text-[var(--text)]' : 'text-[var(--text-secondary)]'}`}
        >
          Transaction History
        </button>
      </div>
      ...
      <div className="mt-6">
        {tab === 'investments' ? (
          investments.length === 0 ? (
            <p className="py-10 text-center text-[var(--text-secondary)]">No investments yet.</p>
          ) : (
          ...
          <p className="py-10 text-center text-[var(--text-secondary)]">No transactions yet.</p>
        ) : (
          <div className="space-y-3">
            {transactions.map((tx) => (
              <Card key={tx.id} className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-[var(--text)]">{tx.description}</p>
                  <p className="text-sm text-[var(--text-secondary)]">
