...
const StudentDashboard = () => {
  ...
  const stats = [
    { icon: DollarSign, label: 'Total Invested', value: `$${(summary?.total_invested || 0).toLocaleString()}`, color: 'text-[var(--accent-primary)]' },
    { icon: PieChart, label: 'Portfolio Value', value: `$${(summary?.current_portfolio_value || 0).toLocaleString()}`, color: 'text-[var(--success)]' },
    { icon: TrendingUp, label: 'Total Returns', value: `$${(summary?.total_returns || 0).toLocaleString()}`, color: 'text-[var(--info)]' },
    { icon: Activity, label: 'Active Investments', value: String(summary?.active_investments || 0), color: 'text-[var(--warning)]' },
  ];
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text)]">Welcome, {student.full_name}</h1>
          <p className="mt-1 text-[var(--text-secondary)]">Your investment overview</p>
        </div>
        <Link to="/merchants">
          <Button><TrendingUp className="h-4 w-4" /> New Investment</Button>
        </Link>
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between">
          ...
          <Link to="/student/portfolio" className="text-sm text-[var(--accent-primary)] hover:underline">View All</Link>
        </div>
        {investments.length === 0 ? (
          <Card className="mt-4 text-center">
            <p className="text-[var(--text-secondary)]">No investments yet.</p>
            <Link to="/merchants" className="mt-2 inline-block">
              <Button variant="outline" size="sm">Browse Merchants</Button>
            </Link>
          </Card>
        ) : (
              ...
              <Link key={inv.id} to={`/student/investments/${inv.id}`}>
                <Card className="flex items-center justify-between transition-colors hover:border-[var(--accent-primary)]">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">{inv.shares} shares at ${inv.price_per_share}/share</p>
                  </div>
